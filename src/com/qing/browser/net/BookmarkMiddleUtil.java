...
public class BookmarkMiddleUtil implements Runnable {

	private static BookmarkMiddleUtil bookmarkUtil;
	...
	private BookmarkMiddleUtil(Context context, Handler handler) {
		this.context = context;
		this.handler = handler;
	}

	public static BookmarkMiddleUtil getInstance(Context context, Handler handler) {
		if (bookmarkUtil == null)
			return new BookmarkMiddleUtil(context, handler);
		return bookmarkUtil;
	}
		...
		try {
			if (Tools.isConnectInternet(context)) {
				poststring = Tools.getPoststring(context)
						+ Tools.getMiddleBookmarkString(context);
